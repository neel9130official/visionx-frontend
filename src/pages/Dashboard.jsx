...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [creditsRes, genRes] = await Promise.all([
          axios.get(`${backendUrl}/api/user/credits`, {
        ...
        if (creditsRes.data.success && genRes.data.success) {
          setStats({
            credits: creditsRes.data.credits || 0,
            imagesGenerated: genRes.data.totalGenerations || 0,
            favoriteStyles: genRes.data.uniqueStyles || 0,
          });
        } else {
  ...
  return (
    <div className="min-h-screen py-20 px-6 text-gray-200">
      <div className="max-w-7xl mx-auto space-y-14">
        ...
        >
          <h1 className="text-3xl font-['Orbitron'] mb-2">
            Welcome back,
            <span className="ml-2 bg-gradient-to-r from-neon to-glow bg-clip-text text-transparent">
              {user?.name}
            </span>
          </h1>
          <p className="text-gray-400">
            Continue creating futuristic visuals with VisionX
          </p>

          {/* Stats */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            {[
              { label: 'Credits Remaining', value: stats.credits },
              { label: 'Images Generated', value: stats.imagesGenerated },
              { label: 'Styles Used', value: stats.favoriteStyles },
            ].map((item, i) => (
              <motion.div
                key={i}
                whileHover={{ scale: 1.05 }}
                className="glass p-6 rounded-2xl border border-neon/20"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { label: 'Create Image', path: '/result' },
            { label: 'Gallery', path: '/gallery' },
            { label: 'Buy Credits', path: '/buy' },
            ...
            <Link
              ...
              className="glass p-8 rounded-2xl border border-white/10
                         hover:border-neon/40 transition-all
                         hover:shadow-[0_0_40px_rgba(0,246,255,0.4)]"
            >
        ...
        <div className="glass p-8 rounded-3xl border border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              'Use lighting, mood & camera angles',
              'Mix styles like cyberpunk + watercolor',
              'Reference artists or eras for depth',
            ].map((tip, i) => (
              <div
                key={i}
                className="border border-neon/20 rounded-xl p-5 text-gray-400"
              >
                {tip}
              </div>
            ))}
