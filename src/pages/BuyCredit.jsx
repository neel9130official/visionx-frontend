// import React, { useContext } from 'react'
// import {assets, plans} from '../assets/assets'
// import { AppContext } from '../context/AppContext'
// import {motion} from 'framer-motion'

// const BuyCredit = () => {

//   const  {user} = useContext(AppContext)

//   return (
//     <motion.div
//     initial={{opacity:0.2, y:100}}
//     transition={{duration:1}}
//     whileInView={{opacity:1, y:0}}
//     viewport={{once:true}}
//     className='min-h-[80vh] text-center pt-14 mb-10'>
//       <button className='border border-gray-400 px-10 py-2 rounded-full mb-6'>Our Plans Coming Soon</button>
//       <h1 className='text-center text-3xl font-medium mb-12 sm:mb-6'>We're Crafting Magic..Stay Tuned!</h1>

//       <div className='flex flex-wrap justify-center gap-6 text-left'>
//         {plans.map((item, index)=>(
//           <div key={index} className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transistion-all duration-500'>
//             <img width={40} src={assets.logo_icon} alt="" />
//             <p className='mt-3 mb-1 font-semibold'>{item.id}</p>
//             <p className='text-sm'>{item.desc}</p>
//             <p className='mt-6'>
//               <span className='text-3xl font-medium'>${item.price} </span> / {item.credits} Credits</p>
//               <button className='w-full bg-gray-800 text-white mt-8 text-sm rounded-md py-2.5 min-w-52'>{user ?'Coming Soon': 'Coming Soon'}</button>
//           </div>
//         ))}
//       </div>

//     </motion.div>
//   )
// }

// export default BuyCredit
import React, { useContext } from 'react';
import { assets, plans } from '../assets/assets';
import { AppContext } from '../context/AppContext';
import { motion } from 'framer-motion';

const BuyCredit = () => {
  const { user } = useContext(AppContext);

  return (
    <motion.div
      initial={{ opacity: 0, y: 80 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8 }}
      viewport={{ once: true }}
      className="min-h-[80vh] text-center py-24 px-6 text-gray-200"
    >
      {/* Badge */}
      <div className="inline-block mb-6 px-8 py-2 rounded-full
                      glass border border-neon/30 text-sm text-gray-300">
        Recharges 
      </div>

      {/* Heading */}
      <h1
  className="
    text-3xl sm:text-4xl font-['Orbitron'] font-semibold mb-12 text-center
    bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500
    bg-clip-text text-transparent
    drop-shadow-[0_0_12px_rgba(0,246,255,0.6)]
  "
>
  We’re building something powerful with AI
</h1>


      {/* Cards */}
      <div className="flex flex-wrap justify-center gap-8 max-w-6xl mx-auto">
        {plans.map((item, index) => (
          <motion.div
            key={index}
            whileHover={{
              scale: 1.06,
              boxShadow: '0 0 50px rgba(0,246,255,0.35)'
            }}
            transition={{ duration: 0.3 }}
            className="
              glass border border-white/10
              rounded-2xl p-8 w-72
              text-left
            "
          >
            {/* Icon */}
            <img
              width={44}
              src={assets.logo_icon}
              alt="VisionX"
              className="mb-4 drop-shadow-[0_0_15px_#00f6ff]"
            />

            {/* Plan Name */}
            <p className="text-lg font-semibold mb-1
                          bg-gradient-to-r from-neon to-glow
                          bg-clip-text text-transparent">
              {item.id}
            </p>

            {/* Description */}
            <p className="text-sm text-gray-400 mb-6">
              {item.desc}
            </p>

            {/* Price */}
            <p className="mb-6">
              <span className="text-3xl font-semibold text-gray-100">
                Rs.{item.price}
              </span>
              <span className="text-sm text-gray-400">
                {' '} / {item.credits} Credits
              </span>
            </p>

            {/* CTA */}
            <button
              disabled
              className="
                w-full py-3 rounded-xl
                bg-gradient-to-r from-neon to-glow
                text-black font-semibold
                opacity-80 cursor-not-allowed
              "
            >
              Coming Soon
            </button>
          </motion.div>
        ))}
      </div>
    </motion.div>
  );
};

export default BuyCredit;
